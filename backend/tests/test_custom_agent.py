...
# ===========================================================================
# 7. Memory isolation: MemoryStore directory paths
# ===========================================================================


class TestMemoryDirPath:
    def test_global_memory_dir(self, tmp_path):
        """None agent_name should return global memory directory."""
        from deerflow.agents.memory.store import MemoryStore

        memory_dir = tmp_path / "memory"
        store = MemoryStore(memory_dir=memory_dir)
        store.load_from_disk()
        assert (tmp_path / "memory") == store.memory_dir

    def test_agent_memory_dir(self, tmp_path):
        """Providing agent_name should return per-agent memory directory."""
        from deerflow.agents.memory.store import MemoryStore

        memory_dir = tmp_path / "agents" / "code-reviewer" / "memory"
        store = MemoryStore(memory_dir=memory_dir)
        store.load_from_disk()
        assert (tmp_path / "agents" / "code-reviewer" / "memory") == store.memory_dir

    def test_different_dirs_for_different_agents(self, tmp_path):
        from deerflow.agents.memory.store import MemoryStore

        dir_global = tmp_path / "memory"
        dir_a = tmp_path / "agents" / "agent-a" / "memory"
        dir_b = tmp_path / "agents" / "agent-b" / "memory"

        store_global = MemoryStore(memory_dir=dir_global)
        store_a = MemoryStore(memory_dir=dir_a)
        store_b = MemoryStore(memory_dir=dir_b)

        assert store_global.memory_dir != store_a.memory_dir
        assert store_global.memory_dir != store_b.memory_dir
