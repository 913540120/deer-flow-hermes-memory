...
def test_memory_disabled_skips_injection():
    """When memory is disabled, _get_memory_context should return empty string."""
    from deerflow.agents.lead_agent import prompt as prompt_module

    app_config = _make_app_config([_make_model("safe-model", supports_thinking=False)])
    app_config.memory = MemoryConfig(enabled=False)

    context = prompt_module._get_memory_context(app_config=app_config)
    assert context == ""
