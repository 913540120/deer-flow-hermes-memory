...
def e2e_env(tmp_path, monkeypatch):
    ...
    # 4. Disable memory injection (avoids file reads for prompt injection)
    from deerflow.config.memory_config import MemoryConfig

    monkeypatch.setattr(
        "deerflow.config.memory_config._global_memory_config",
...
class TestMemoryAccess:
    ...
    def test_get_memory_config_fields(self, e2e_env):
        ...
        assert "memory_char_limit" in result
        assert "user_char_limit" in result
