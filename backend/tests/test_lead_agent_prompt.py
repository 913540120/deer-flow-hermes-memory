...
def test_get_memory_context_uses_explicit_app_config_without_global_config(monkeypatch, tmp_path):
    explicit_config = SimpleNamespace(
        memory=SimpleNamespace(enabled=True, injection_enabled=True, memory_char_limit=2200, user_char_limit=1375),
    )
    ...
    # Set up a fake paths that points to tmp_path
    ...
    # Create the memory directory with some content so we get a non-empty result
    memory_dir = tmp_path / "users" / "user-1" / "agents" / "agent-a" / "memory"
    memory_dir.mkdir(parents=True)
    (memory_dir / "MEMORY.md").write_text("- I know this test works")

    context = prompt_module._get_memory_context("agent-a", app_config=explicit_config)

    assert "<memory>" in context
    assert "I know this test works" in context
