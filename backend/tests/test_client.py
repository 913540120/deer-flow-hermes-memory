...
class TestConfigQueries:
    ...
    def test_get_memory(self, client, tmp_path):
        from types import SimpleNamespace

        config = SimpleNamespace(memory_char_limit=2200, user_char_limit=1375)
        ...
        with (
            ...
            patch("deerflow.config.paths.get_paths", return_value=fake_paths),
            patch("deerflow.runtime.user_context.get_effective_user_id", return_value="test-user"),
        ):
            result = client.get_memory()
        assert "memory" in result
        assert "user" in result
        assert result["memory"]["entries"] == []
        assert result["user"]["entries"] == []
...
class TestMemoryManagement:
    def test_reload_memory(self, client, tmp_path):
        from types import SimpleNamespace
    ...
    def test_clear_memory(self, client, tmp_path):
        from types import SimpleNamespace
        ...
        memory_dir = tmp_path / "users" / "test-user" / "memory"
        store = MemoryStore(memory_dir=memory_dir)
        store.load_from_disk()
        store.add("memory", "test entry")
        # add() persists to disk automatically
    ...
    def test_get_memory_config(self, client):
        config = MagicMock()
        config.enabled = True
        config.storage_path = ".deer-flow/memory"
        config.injection_enabled = True
        config.memory_char_limit = 2200
        config.user_char_limit = 1375

        with patch("deerflow.config.memory_config.get_memory_config", return_value=config):
            result = client.get_memory_config()

        assert result["enabled"] is True
        assert result["memory_char_limit"] == 2200

    def test_get_memory_status(self, client, tmp_path):
        from types import SimpleNamespace
...
class TestScenarioMemoryWorkflow:
    """Scenario: Memory query → add → reload → status check."""

    def test_memory_full_lifecycle(self, client, tmp_path):
        """get_memory → add → reload → get_status covers the full memory API."""
        from types import SimpleNamespace
        ...
        assert mem["memory"]["entries"] == []

        # Write directly to the store, then reload
        from deerflow.agents.memory.store import MemoryStore
        ...
        assert any("User likes Python" in e for e in refreshed["memory"]["entries"])
        ...
        assert any("User likes Python" in e for e in status["data"]["memory"]["entries"])
...
class TestGatewayConformance:
    ...
    def test_get_memory_config(self, client):
        ...
        mem_cfg.storage_path = ".deer-flow/memory"
        mem_cfg.injection_enabled = True
        mem_cfg.memory_char_limit = 2200
        mem_cfg.user_char_limit = 1375
        ...
        assert parsed.memory_char_limit == 2200
    ...
    def test_get_memory_status(self, client, tmp_path):
        ...
        assert parsed.data.memory.entries == []
