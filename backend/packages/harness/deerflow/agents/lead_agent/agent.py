...
def _build_middlewares(
    ...
    *,
    ...
    Args:
        ...
        agent_name: If provided, used for per-agent memory directory isolation.
...
def _make_lead_agent(config: RunnableConfig, *, app_config: AppConfig):
    ...
    # Register memory tool when memory is enabled
    if resolved_app_config.memory.enabled:
        ...
        store = MemoryStore(
            ...
            memory_char_limit=resolved_app_config.memory.memory_char_limit,
            user_char_limit=resolved_app_config.memory.user_char_limit,
        )
        ...
        if resolved_app_config.memory_provider.enabled and resolved_app_config.memory_provider.name:
        ...
        _memory_manager.initialize_all(config.get("configurable", {}).get("thread_id", ""))
        ...
        for _schema in _memory_manager.get_all_tool_schemas():
            if _schema["name"] != "memory":
                extra_tools.append(_create_routed_tool(_memory_manager, _schema))
