"""Bounded curated memory with file persistence — ported from Hermes.

Provides two file-backed stores:
...
class MemoryStore:
    ...
    def _file_lock(path: Path):
        ...
        Uses a separate .lock sidecar file so the memory file itself can still
    ...
    def _validate_target(self, target: str) -> dict[str, Any] | None:
        """Return an error dict if target is invalid, else None."""
    ...
    def add(self, target: str, content: str) -> dict[str, Any]:
        ...
        validation = self._validate_target(target)
        if validation:
            return validation
    ...
    def replace(self, target: str, old_text: str, new_content: str) -> dict[str, Any]:
        ...
        with self._file_lock(self._path_for(target)):
            ...
            if new_total > limit:
                return {
                    "success": False,
                    "error": (f"Replacement would put memory at {new_total:,}/{limit:,} chars. Shorten the new content or remove other entries first."),
                }
    ...
    def get_live_entries(self, target: str) -> list[str]:
        """Return a copy of the live entries for the given target."""
        return list(self._entries_for(target))

    def get_usage(self, target: str) -> dict[str, Any]:
        """Return usage info: entries count, chars, and limit."""
        entries = self._entries_for(target)
    ...
    def clear(self, target: str) -> dict[str, Any]:
        """Clear all entries for the given target and persist."""
        validation = self._validate_target(target)
        if validation:
            return validation
    ...
    def _write_file(path: Path, entries: list[str]) -> None:
        ...
        Uses tempfile + fsync + os.replace so readers always see either the
